...
export const agregarUsers = async (req,res) => {
    try {
        ...
        if (!name || !email || !password) {
            return res.status(400).json({mensaje:'No pueden estar vacios,por favor ingrese datos requeridos'})
        }
...
export const editarUsers = async (req,res) => {
    try {
            ...
            console.log(error);
            return res.status(500).json({mensaje:'Error al poder modificar usuarios',error:error.message
            })
        }
}
export const getPorIdUsers = async (req,res) => {
    try {
        ...
        if (idUsers) {
            res.status(201).json({mensaje:'Aqui esta el usuario que buscaste',idUsers});
        
        } else {
            res.status(404).json({mensaje:'El usuario no fue encontrado'});
        }
    } catch (error) {
        res.status(500).json({mensaje:'Error al poder ver por id al usuario',error:error.message})
    }
}
export const eliminarUsers = async (req,res) =>{
    try {
        ...
        if (borrarUsers) {
            res.status(201).json({mensaje:'Este usuario fue borrado con exito',borrarUsers})
        } else {
            res.status(404).json({mensaje:'Este usuario no fue encontrado',});
        }
    } catch (error) {
        res.status(500).json({mensaje:'Hubo error al eliminar usuarios',error:error.message
        })
    }
}